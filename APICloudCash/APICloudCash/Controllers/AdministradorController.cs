...
namespace APICloudCash.Controllers
{
    public class AdministradorController : ApiController
    {
        ...
        public string RegistrarServicio(entServicios entServicio)
        {
            try
            {
                using (var context = new DBCC())
                {
                    var user = new TipoServicios();

                    user.descripcion = entServicio.descripcion;
                    user.monto = entServicio.monto;
                    user.activo = true;

                    context.TipoServicios.Add(user);
                    context.SaveChanges();
        ...
        public List<TipoServicios> ListarServicios()
        {
            try
            {
                using (var context = new DBCC())
                {
                    context.Configuration.LazyLoadingEnabled = false;
                    return (from x in context.TipoServicios
                            select x).ToList();
                }
            }
            catch (Exception)
            {
                return new List<TipoServicios>();
            }
        }

        [HttpPut]
        [Route("ActualizarEstadoServicio")]
        public string ActualizarEstadoServicio(entServicios servicio)
        {
            try
            {
                using (var context = new DBCC())
                {
                    var datos = (from x in context.TipoServicios
                                 where x.id_TipoServicio == servicio.id_TipoServicio
                                 select x).FirstOrDefault();
